...
var Inventory = (function(){
  ...
  document.getElementById('formAddItem').addEventListener('submit', async e => {
    ...
    if (id) {
      ...
      await db.ref('stock/' + id).update({ name: payload.name, quantity: payload.stock, updatedAt: Date.now() });
      alert('Item diupdate');
    } else {
      payload.createdAt = Date.now();
      const newRef = await invRef.push(payload);
      const newId = newRef.key;
      // create stock entry with same key as inventory id
      await db.ref('stock/' + newId).set({
        itemId: newId,
        name: payload.name,
        quantity: payload.stock || 0,
        createdAt: payload.createdAt
      });
      alert('Item ditambahkan dan stock tercatat');
    }
    clearItemForm();
  });
